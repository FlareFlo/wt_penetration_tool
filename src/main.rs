...
fn main() {
	...
	const SQUARE_SIZE_X: u32 = 5; //X Dimensions of the scan square
	const SQUARE_SIZE_Y: u32 = 5; //Y Dimensions of the scan square

	const NUM_SQUARES_X: u32 = 5;
	const NUM_SQUARES_Y: u32 = 5;
...
fn get_frame(capturer: &mut Capturer) -> scrap::Frame {
	loop {
		if let Ok(frame) = capturer.frame() {
			//Unsafe due to borrow checker-bug
